...
const About = () => {
  ...
  useGSAP(() => {
    ...
    tl.to(".about-heading", { x: 0, opacity: 1, duration: 1, ease: "power3.out" })
      .to(".about-stat", { y: 0, opacity: 1, stagger: 0.2, duration: 0.5 }, "-=0.8")
      .to(".about-bio", { y: 0, opacity: 1, duration: 0.8 }, "-=0.5");
  }, { scope: container });

  return (
    <section id="about" ref={container} className="py-32 relative bg-white text-black">
      <div className="absolute -top-20 right-20 z-10 overflow-hidden">
        <h2 className="about-heading font-black text-[11vw] leading-none tracking-tighter uppercase text-neon-pink mix-blend-difference translate-x-[200px] opacity-0">
          ABOUT
        </h2>
      </div>
